...
describe("Calendar Data Service", () => {
  ...
  describe("aggregateTradesByStrategy", () => {
    ...
    it("should match backtest and actual by strategy mapping", () => {
      const dayData = createDayData({
        date: "2025-01-15",
        backtestTrades: [createBacktestTrade({ strategy: "BT Strategy", pl: 100 })],
        actualTrades: [createActualTrade({ strategy: "Live Strategy", pl: 90 })],
        hasBacktest: true,
        hasActual: true,
      });
    ...
    it("should include unmatched actual strategies separately", () => {
      const dayData = createDayData({
        date: "2025-01-15",
        backtestTrades: [createBacktestTrade({ strategy: "Strategy A", pl: 100 })],
        actualTrades: [createActualTrade({ strategy: "Strategy B", pl: 200 })],
        hasBacktest: true,
        hasActual: true,
      });
  ...
  describe("groupDatesByWeek", () => {
    it("should group dates by ISO week number", () => {
      ...
      const weekCounts = Array.from(grouped.values())
        .map((v) => v.length)
        .sort();
    ...
    describe("getScaledDayBacktestPl", () => {
      ...
      it("should scale DOWN to actual in toReported mode", () => {
        ...
        const strategyMatches: StrategyMatch[] = [
          {
            backtestStrategy: "Test Strategy",
            actualStrategy: "Test Strategy",
            isAutoMatched: false,
          },
        ];
    ...
    describe("Scaling consistency - verifying centralized logic", () => {
      ...
      it("should handle unmatched strategies in toReported mode", () => {
        ...
        const dayData = createDayData({
          ...
          actualTrades: [createActualTrade({ strategy: "Matched", numContracts: 1, pl: 25 })],
          hasBacktest: true,
          hasActual: true,
        });
        ...
        const strategyMatches: StrategyMatch[] = [
          { backtestStrategy: "Matched", actualStrategy: "Matched", isAutoMatched: false },
        ];
