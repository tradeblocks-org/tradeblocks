...
describe("Static Datasets Store - Match Stats Caching", () => {
  ...
  describe("computeMatchStats", () => {
    it("returns cached stats if already cached", async () => {
      ...
      const store = useStaticDatasetsStore.getState();
      const result = await store.computeMatchStats(
        "dataset-1",
        "block-1",
        mockTrades,
        "nearest-before",
    ...
    it("computes and caches stats when not cached", async () => {
      ...
      const cachedResult = useStaticDatasetsStore
        .getState()
        .getMatchStats("dataset-1", "block-1", "nearest-before");
    ...
    it("clears computing flag after completion", async () => {
      ...
      const isComputing = useStaticDatasetsStore
        .getState()
        .isComputingMatchStats("dataset-1", "block-1", "nearest-before");
  ...
  describe("invalidateMatchStatsForBlock", () => {
    it("removes all cached stats for a specific block", () => {
      const stats1: DatasetMatchStats = {
        totalTrades: 10,
        matchedTrades: 8,
        matchPercentage: 80,
        outsideDateRange: 2,
      };
      const stats2: DatasetMatchStats = {
        totalTrades: 20,
        matchedTrades: 18,
        matchPercentage: 90,
        outsideDateRange: 0,
