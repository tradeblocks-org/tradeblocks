...
const mockGetDailyLogsByBlock = db.getDailyLogsByBlock as jest.MockedFunction<
  typeof db.getDailyLogsByBlock
>;
const mockSaveWalkForwardAnalysis = db.saveWalkForwardAnalysis as jest.MockedFunction<
  typeof db.saveWalkForwardAnalysis
>;
const mockGetWalkForwardAnalysesByBlock = db.getWalkForwardAnalysesByBlock as jest.MockedFunction<
  typeof db.getWalkForwardAnalysesByBlock
>;
...
describe("strategy weight sweep config", () => {
  ...
  it("updates strategy weight config", async () => {
    ...
    const config = useWalkForwardStore
      .getState()
      .strategyWeightSweep.configs.find((c) => c.strategy === "Straddle");
  ...
  it("limits strategies in fullRange mode to 3", async () => {
    ...
    const enabledCount = useWalkForwardStore
      .getState()
      .strategyWeightSweep.configs.filter((c) => c.enabled).length;
...
describe("trade frequency calculation", () => {
  ...
  it("calculates frequency for sparse trades", () => {
    ...
    const trades = createTradesWithDates(["2024-01-01", "2024-02-01", "2024-03-01"]);
