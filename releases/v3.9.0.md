...
reached its stop_. The same held on profit targets, trailing stops and stepped
profit actions.
...
Thresholds are now derived in exact decimal arithmetic, so the threshold you set,
the threshold reported, and the threshold compared against are one number.
Comparisons remain inclusive. **An exit analysis that sat exactly on a percentage
threshold may now report a trigger where it previously reported none.** That is
the corrected answer, but re-run any saved analysis you are relying on.

The same exactness now covers the P&L this package derives from your leg prices,
so a three-cent move reaches a three-cent target and a two-cent drop reaches a
two-cent trailing stop. Previously those landed on values like
`0.029999999999996696` and fell short of the threshold they had actually met.
Per-leg-group P&L is derived the same way.

Reported P&L figures keep their existing two-decimal presentation.
...
- **A threshold that cannot be represented is refused with a clear message**
  naming the field, instead of silently becoming a different exit decision. Two
  cases are refused: one so small it would round away to zero — a zero threshold
  is met by every position, so a stop nobody could reach would have fired
  immediately — and one beyond the largest amount the analysis represents. This
  applies wherever the threshold comes from, including one derived from a
  percentage and an entry cost. No field a trigger type ignores is checked, so
  requests that worked before still work.
...
- **An exit that matched the actual exit is now reported as such.** A trigger and
  ...
  now reads `Trigger was the same.` Zero is neither better nor worse, and exact
  thresholds make the equal case reachable rather than theoretical.
