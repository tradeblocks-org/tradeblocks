...
- **Reported figures now match the comparison that produced them.** The fired
  event, the analysis summary, per-group summaries and the batch aggregate
  previously carried the raw accumulated value while the detail line carried the
  exact one — so an answer could say `$7.275 >= target $7.275` and report `$7.27`
  beside it. They now report the same figure the comparison used.
...
- **An unusable monetary amount is now refused with a clear message** naming the
  field, instead of silently becoming a different exit decision. Two cases are
  refused: an amount so small it would round away to zero — a zero threshold is
  met by every position, so a stop nobody could reach would have fired
  immediately — and one beyond the largest amount the analysis represents. This
  applies wherever the amount comes from, including a threshold derived from a
  percentage and an entry cost. Amounts you could actually have meant are
  unaffected, and no field a trigger type ignores is checked, so requests that
