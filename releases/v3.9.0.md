...
Thresholds this analysis derives from your configuration are now calculated in
exact decimal arithmetic, so the threshold reported and the threshold compared
against are one number. This covers percentage-of-entry-cost targets and stops
and profit-action step arms and stops. Configured dollar thresholds are validated
and carried through the same exact domain. Comparisons remain inclusive. **An
exit analysis that sat exactly on one of these thresholds may now report a
trigger where it previously reported none.** That is the corrected answer, but
re-run any saved analysis you are relying on.
...
- **A threshold that cannot be represented is refused with a clear message**
  naming the field, instead of silently becoming a different exit decision. This
  includes values that would round to zero, values such as `$0.0000006` that would
  round to a different non-zero amount, and values beyond the largest amount the
  analysis represents. The refusal applies wherever the threshold comes from,
  including one derived from a percentage and an entry cost. No field a trigger
  type ignores is checked, so public request schemas remain unchanged.
