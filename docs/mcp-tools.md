...
| `decompose_greeks`      | Decompose P&L into delta, gamma, theta, vega, charm, vanna, and residual. Automatic numerical fallback when the model-based residual exceeds 80% of the gross attribution flow (the sum of absolute factor totals, residual included) — measured against gross flow rather than net P&L so a legitimate multi-day step on a hedged position does not trip the fallback. Per-leg-group vega attribution for calendar strategies. |
