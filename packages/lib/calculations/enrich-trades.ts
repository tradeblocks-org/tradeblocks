 ...
 */
function buildDailyCustomFieldsMap(
  dailyLogs: DailyLogEntry[],
...
function enrichSingleTrade(
  ...
  const rMultiple =
    mfeMaePoint?.mae && mfeMaePoint.mae > 0 ? trade.pl / mfeMaePoint.mae : undefined;
...
export function enrichTrades(trades: Trade[], options?: EnrichTradesOptions): EnrichedTrade[] {
  ...
  const mfeMaeMap = new Map<number, MFEMAEDataPoint>(mfeMaeData.map((d) => [d.tradeNumber - 1, d]));
