...
export class WalkForwardAnalyzer {
  ...
  async analyze(options: AnalyzeOptions): Promise<WalkForwardComputation> {
    ...
    for (let index = 0; index < windows.length; index++) {
      ...
      if (inSampleTrades.length < minInSample || outSampleTrades.length < minOutSample) {
        const reason =
          inSampleTrades.length < minInSample
            ? ("insufficient_is_trades" as const)
            : ("insufficient_oos_trades" as const);
        const detail =
          inSampleTrades.length < minInSample
      ...
      const inSampleInitialCapital =
        PortfolioStatsCalculator.calculateInitialCapital(inSampleTrades);
      const outSampleInitialCapital =
        PortfolioStatsCalculator.calculateInitialCapital(outSampleTrades);
      ...
      for (const params of combinationIterator.values) {
        ...
        if (enforceDiversificationConstraints && diversificationConfig) {
          ...
          if (
            inSampleDivMetrics &&
            !this.isDiversificationAcceptable(inSampleDivMetrics, diversificationConfig)
          ) {
            continue;
          }
        }
  ...
  private calculateDiversificationMetrics(
    ...
    return {
      ...
      maxTailDependencePair: (tailRiskResult?.analytics.highestJointTailRisk.pair ?? ["", ""]) as [
        string,
        string,
      ],
  ...
  private calculateSummary(periods: WalkForwardPeriodResult[]): WalkForwardSummary {
    ...
    const inSampleValues = periods
      .map((p) => p.targetMetricInSample)
      .filter((value) => Number.isFinite(value));
    const outSampleValues = periods
      .map((p) => p.targetMetricOutOfSample)
      .filter((value) => Number.isFinite(value));
