...
export function calculateThresholdAnalysis(
  ...
  const tradesWithX = trades
    .map((trade) => ({
      ...
      pl: trade.pl ?? 0,
      plPct:
        trade.premiumEfficiency ??
        (trade.premium && trade.premium !== 0 && trade.numContracts
          ? (trade.pl / Math.abs(trade.premium * trade.numContracts)) * 100
          : 0),
      rom: trade.rom ?? 0,
    }))
  ...
  const dataPoints: ThresholdDataPoint[] = sampleXValues.map((threshold) => {
    ...
    const cumulativePlPct =
      totalPl !== 0 ? (plBelow / totalPl) * 100 : (tradesBelow.length / totalTrades) * 100;

    // Average P/L above/below threshold
    const avgPlAbove =
      tradesAbove.length > 0
        ? tradesAbove.reduce((sum, t) => sum + t.pl, 0) / tradesAbove.length
        : null;
    const avgPlBelow =
      tradesBelow.length > 0
