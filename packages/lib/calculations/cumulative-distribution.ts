...
export function findOptimalThreshold(
  ...
  for (const point of analysis.points) {
    const sampleSize = metric.includes("Above") ? point.tradesAtOrAbove : point.tradesAtOrBelow;
...
export function calculateMultipleDistributions(
  ...
  for (const field of fields) {
    results.set(
      field,
      calculateCumulativeDistribution(trades, field, numBuckets, derivedFieldsMap),
    );
  }
