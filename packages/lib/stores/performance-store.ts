...
import { deriveGroupedLegOutcomes, type GroupedLegOutcomes } from "../utils/performance-helpers.ts";
      ...
      if (isDefaultView) {
        ...
        if (cachedSnapshot) {
          ...
          const hasEquityCurve =
            cachedSnapshot.chartData.equityCurve && cachedSnapshot.chartData.equityCurve.length > 0;
