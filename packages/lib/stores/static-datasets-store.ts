 ...
 */
export function makeMatchStatsCacheKey(
  datasetId: string,
  blockId: string,
...
interface StaticDatasetsState {
  ...
  getMatchStats: (
    ...
    blockId: string,
    matchStrategy: MatchStrategy,
  ) => DatasetMatchStats | null;
  computeMatchStats: (
    datasetId: string,
    blockId: string,
    trades: Trade[],
    matchStrategy: MatchStrategy,
  ) => Promise<DatasetMatchStats | null>;
  isComputingMatchStats: (
    datasetId: string,
    blockId: string,
    matchStrategy: MatchStrategy,
  ) => boolean;
...
export const useStaticDatasetsStore = create<StaticDatasetsState>((set, get) => ({
  ...
  updateMatchStrategy: async (id, strategy) => {
    try {
      ...
      set((state) => ({
        datasets: state.datasets.map((d) => (d.id === id ? { ...d, matchStrategy: strategy } : d)),
      }));
  ...
  renameDataset: async (id, newName) => {
    ...
    try {
      ...
      set((state) => ({
        datasets: state.datasets.map((d) => (d.id === id ? { ...d, name: trimmedName } : d)),
      }));
...
export function useStaticDataset(id: string): StaticDataset | undefined {
  return useStaticDatasetsStore((state) => state.datasets.find((d) => d.id === id));
}
