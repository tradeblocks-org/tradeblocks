...
describe("resolveProviderCapabilities", () => {
  ...
  test("quoteHydration is true for Massive whenever fetchQuotes exists, regardless of tier", () => {
    ...
    expect(
      resolveProviderCapabilities(provider, { MASSIVE_DATA_TIER: "ohlc" }).quoteHydration,
    ).toBe(true);
    expect(
      resolveProviderCapabilities(provider, { MASSIVE_DATA_TIER: "trades" }).quoteHydration,
    ).toBe(true);
    expect(
      resolveProviderCapabilities(provider, { MASSIVE_DATA_TIER: "quotes" }).quoteHydration,
    ).toBe(true);
