...
describe("bsPrice", () => {
  ...
  test("T=0 returns intrinsic value for call", () => {
    expect(bsPrice("call", 110, 100, 0, r, q, 0.2)).toBeCloseTo(10, 5);
    expect(bsPrice("call", 90, 100, 0, r, q, 0.2)).toBeCloseTo(0, 5);
  });

  test("T=0 returns intrinsic value for put", () => {
    expect(bsPrice("put", 90, 100, 0, r, q, 0.2)).toBeCloseTo(10, 5);
    expect(bsPrice("put", 110, 100, 0, r, q, 0.2)).toBeCloseTo(0, 5);
  });
...
describe("solveIV", () => {
  ...
  test("converges to ~0.20 for ATM call", () => {
    const marketPrice = bsPrice("call", S, K, T, r, q, 0.2);
  ...
  test("converges to ~0.20 for ATM put", () => {
    const marketPrice = bsPrice("put", S, K, T, r, q, 0.2);
  ...
  test("converges for OTM put", () => {
    const marketPrice = bsPrice("put", 110, 100, 0.5, r, q, 0.3);
...
describe("computeLegGreeks", () => {
  test("returns GreeksResult with all fields for valid inputs", () => {
    ...
    const price = bsPrice("call", 100, 100, 1.0, 0.045, 0.015, 0.2);
    ...
    expect(result.iv!).toBeCloseTo(0.2, 1);
  });
  ...
  test("converts DTE to years internally (dte/365)", () => {
    ...
    expect(result365.iv!).toBeCloseTo(0.2, 2);
  });

  test("handles put type correctly", () => {
    const putPrice = bsPrice("put", 100, 100, 1.0, 0.045, 0.015, 0.2);
...
describe("computeLegGreeks model selection", () => {
  ...
  test("dte=0.3 (between 0.1 and old 0.5 threshold) now uses Black-Scholes — iv is log-normal (0-1 range)", () => {
    ...
    const T = 0.3 / 365;
    const price = bsPrice("call", 100, 100, T, 0.045, 0.015, 0.2);
