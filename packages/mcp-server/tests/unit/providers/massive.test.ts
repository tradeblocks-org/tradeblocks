...
const VALID_RESPONSE = {
  ...
  resultsCount: 1,
  adjusted: false,
  results: [{ v: 1000, vw: 20.5, o: 20.0, c: 21.0, h: 21.5, l: 19.5, t: 1736253000000, n: 50 }],
  status: "OK",
  request_id: "req-123",
};
...
describe("MassiveAggregateResponseSchema", () => {
  ...
  it("accepts a response with empty results array", () => {
    expect(
      MassiveAggregateResponseSchema.safeParse({
        ...VALID_AGG_RESPONSE,
        results: [],
        resultsCount: 0,
      }).success,
  ...
  it("rejects a response where results is not an array", () => {
    expect(
      MassiveAggregateResponseSchema.safeParse({ ...VALID_AGG_RESPONSE, results: "not-an-array" })
        .success,
    ).toBe(false);
  });
});
  ...
  describe("response parsing", () => {
    ...
    it("returns empty array when results is empty", async () => {
      fetchSpy.mockResolvedValueOnce(
        mockResponse({ ...VALID_RESPONSE, results: [], queryCount: 0, resultsCount: 0 }),
      );
      const rows = await provider.fetchBars({
        ticker: "VIX",
        from: "2025-01-01",
...
describe("MassiveProvider.fetchOptionSnapshot", () => {
  ...
  describe("pagination", () => {
    ...
    it("throws on repeated pagination cursor", async () => {
      ...
      fetchSpy
        ...
        .mockResolvedValueOnce(mockResponse(repeating));
      await expect(provider.fetchOptionSnapshot({ underlying: "SPX" })).rejects.toThrow(
    ...
    it("rejects response with missing required fields", async () => {
      fetchSpy.mockResolvedValueOnce(
        mockResponse({
          request_id: "req-001",
          status: "OK",
          results: [{ break_even_price: 100, implied_volatility: 0.2, open_interest: 50 }],
        }),
      );
      await expect(provider.fetchOptionSnapshot({ underlying: "SPX" })).rejects.toThrow(
        "validation failed",
  ...
  describe("filter params", () => {
    it("includes strike_price and expiration_date filters in URL", async () => {
      fetchSpy.mockResolvedValueOnce(mockResponse(makeSnapshotResponse()));
      await provider.fetchOptionSnapshot({
        underlying: "SPX",
...
describe("MassiveQuoteSchema", () => {
  ...
  it("rejects a quote with string sip_timestamp", () => {
    expect(
      MassiveQuoteSchema.safeParse({ ...VALID_QUOTE, sip_timestamp: "not-a-number" }).success,
    ).toBe(false);
  });
...
describe("MassiveQuotesResponseSchema", () => {
  ...
  it("accepts a response with next_url", () => {
    const resp = {
      status: "OK",
      request_id: "req-quotes-002",
      results: [VALID_QUOTE],
      next_url: "https://api.massive.com/v3/quotes/next?cursor=abc",
    };
    expect(MassiveQuotesResponseSchema.safeParse(resp).success).toBe(true);
  });
...
function makeQuotesResponse(
  quotes: Array<{ bid: number; ask: number; nanos: number }>,
  nextUrl?: string,
) {
  return {
    status: "OK",
...
describe("Quotes enrichment", () => {
  ...
  it("fetchBars returns raw OHLCV bars without bid/ask enrichment", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(mockResponse(makeOptionBarsResponse()));
  ...
  it("bars without matching quotes retain undefined bid/ask", async () => {
    ...
    fetchSpy
      ...
      .mockResolvedValueOnce(
        mockResponse(makeQuotesResponse([{ bid: 12.4, ask: 13.6, nanos: differentNanos }])),
      );
  ...
  it("returns bars without bid/ask when quotes endpoint returns 403 (tier restriction)", async () => {
    fetchSpy.mockResolvedValueOnce(mockResponse(makeOptionBarsResponse())).mockResolvedValueOnce(
...
describe("MassiveProvider.fetchQuotes — tier-aware endpoint selection", () => {
  ...
  it("synthesizes bid=ask=close for every minute returned by /v2/aggs", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(
      mockResponse({
        ...
        results: [
          ...
          { v: 60, vw: 13.4, o: 13.2, c: 13.5, h: 13.6, l: 13.1, t: 1736260260000, n: 12 },
        ],
        status: "OK",
    ...
    expect(quotes.get("2025-01-07 09:30")!.bid).toBe(13.2);
    expect(quotes.get("2025-01-07 09:31")!.bid).toBe(13.5);
  });
  ...
  it("filters out bars outside RTH (09:30–16:00 ET)", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(
      mockResponse({
        ...
        adjusted: false,
        results: [
          { v: 10, vw: 13.0, o: 12.8, c: 13.1, h: 13.2, l: 12.7, t: 1736260140000, n: 5 },
          { v: 50, vw: 13.0, o: 12.8, c: 13.2, h: 13.5, l: 12.5, t: 1736260200000, n: 10 },
          { v: 5, vw: 13.0, o: 12.8, c: 13.3, h: 13.4, l: 12.9, t: 1736283660000, n: 3 },
        ],
        status: "OK",
