...
const SAMPLE_TRADE_ROW_1 =
  "2024-01-02,09:35:00,2024-01-02,15:30:00,2.50,0.50,SPX 4800P/4750P,250,1,200,Sync Test Strategy,1.50,1.50,Target,10200,5000";
const SAMPLE_TRADE_ROW_2 =
  "2024-01-03,09:35:00,2024-01-03,15:45:00,2.75,0.25,SPX 4820P/4770P,275,1,250,Sync Test Strategy,1.50,1.50,Target,10450,5000";
const SAMPLE_TRADE_ROW_3 =
  "2024-01-04,09:35:00,2024-01-04,14:00:00,2.25,0.10,SPX 4900P/4850P,225,2,430,Sync Test Strategy,3.00,3.00,Target,10680,5000";
const SAMPLE_REPORTING_ROW_1 =
  "2024-01-02,09:35:00,2024-01-02,15:30:00,2.50,0.50,SPX 4800P/4750P,250,1,180,Sync Test Strategy,Target,0.50";
...
describe("Sync Layer Integration", () => {
  ...
  describe("Change Detection", () => {
    ...
    it("detects changed blocks correctly", async () => {
      ...
      await createBlockWithTrades(testDir, "changed-block", [
        SAMPLE_TRADE_ROW_1,
        SAMPLE_TRADE_ROW_2,
      ]);
    ...
    it("skips unchanged blocks (not reprocessed)", async () => {
      // Create and sync a block
      await createBlockWithTrades(testDir, "unchanged-block", [
        SAMPLE_TRADE_ROW_1,
        SAMPLE_TRADE_ROW_2,
      ]);
  ...
  describe("Transaction Rollback", () => {
    ...
    it("cleans up data on sync failure for previously-synced block", async () => {
      ...
      await createBlockWithTrades(testDir, "cleanup-test", [
        SAMPLE_TRADE_ROW_2,
        SAMPLE_TRADE_ROW_3,
      ]);
  ...
  describe("Edge Cases", () => {
    ...
    it("syncBlock returns deleted for removed folder", async () => {
      // Create and sync a block
      const blockPath = await createBlockWithTrades(testDir, "sync-block-test", [
        SAMPLE_TRADE_ROW_1,
      ]);
    ...
    it("handles block with hidden files correctly", async () => {
      ...
      await fs.writeFile(
        path.join(testDir, ".hidden-folder", "tradelog.csv"),
        `${CSV_HEADERS}\n${SAMPLE_TRADE_ROW_2}`,
      );
    ...
    it("sequential syncs preserve data consistency", async () => {
      ...
      // Create a block
      await createBlockWithTrades(testDir, "sequential-sync-block", [
        SAMPLE_TRADE_ROW_1,
        SAMPLE_TRADE_ROW_2,
      ]);
